  ...
  const components = {
    code: ({ node, inline, className, children, ...props }: any) => {
      ...
      return !inline && match ? (
        <pre
          {...props}
          className={`${className} mt-2 w-[80dvw] overflow-x-scroll rounded-lg bg-accent p-3 text-sm md:max-w-[500px]`}
        >
          <code className={match[1]}>{children}</code>
        </pre>
      ) : (
        <code
          className={`${className} rounded-md bg-accent px-1 py-0.5 text-sm`}
          {...props}
        >
          {children}
        </code>
