...
export function Block({
  ...
  return (
    ...
    >
      {!isMobile && (
        <motion.div
          className="relative h-dvh w-1/2 shrink-0 bg-muted dark:bg-background"
        ...
        >
          <AnimatePresence>
            {!isCurrentVersion && (
              <motion.div
                className="absolute left-0 top-0 z-50 h-dvh w-1/2 bg-zinc-900/50"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              />
      ...
      <motion.div
        className="fixed flex h-dvh flex-col overflow-y-scroll border-l border-border bg-background dark:bg-muted"
        initial={
          isMobile
            ? {
                opacity: 0,
